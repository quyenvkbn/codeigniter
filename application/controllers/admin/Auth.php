		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				if ($this->ion_auth->is_admin())
				{
					...
					//Hiện tại admin đang cho phép thay đổi cả group admin quyền cao nhất đây là code update để không cho thêm quyền admin vào các member khác. chỉ có 1 admin cao nhất còn lại là các (general admin để sửa thông tin nội dung web).
					if($this->ion_auth->in_group([1],$id)){
						if(empty($groupData)){
							$groupData = array();
						}
						$groupData[] = 1;
